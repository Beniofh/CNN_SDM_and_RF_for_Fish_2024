...
def Figure_pred(df_true,
    ...
    cbar.set_label('mean log abundance')
    ...
    if global_min_dif < 0 and global_max_dif > 0:
        ...
        sm.set_array([])
        cbar = plt.colorbar(sm, cax=cax)
    cbar.set_label('diff. of mean log abundance')
    # Add world map in background
    cx.add_basemap(ax, crs=cells.crs,source=cx.providers.OpenStreetMap.Mapnik)
...
# predefined map type 
map_type = 'West_Med_15_15' #Alboran_Sea_5_5 ; #West_Med_15_15 ; Mallorca_Ibiza_5_5 ; Perpignan_Mataro_2_2 ; Alboran_Sea_Zoom_6_6 ; Mallorca_Ibiza_Zoom_4_4
# Test map
test_map = False # True if you want to test the map with a single species, False to plot all species
test_name = 'Serranus cabrilla' #'Apogon imberbis'#'Serranus scriba'
# File name with percentage
file_name_with_percentage = False #True
...
for sp in tqdm(species_list, desc="Processing species"):
    ...
    if file_name_with_percentage == True:
        if percentage_non_zero_rounded < 10:
            output_path_fig_2 = f'{path_output_dir}0{percentage_non_zero_rounded}_{sp.replace(" ", "_")}.png'
        else:
            output_path_fig_2 = f'{path_output_dir}{percentage_non_zero_rounded}_{sp.replace(" ", "_")}.png'
    else:
        output_path_fig_2 = f'{path_output_dir}{sp.replace(" ", "_")}.png'
